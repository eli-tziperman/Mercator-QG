...
-r\nabla^2\psi=-r\zeta',
\]
and a biharmonic friction term proportional to
\[
+\kappa\nabla^4\psi
=
+\kappa\nabla^2(\nabla^2\psi)
...
\right].
\]
Then include an external forcing \(F(x,y,t)\). The forced-damped linearized equation is
\[
\boxed{
\begin{aligned}
\mathcal D_0
...
The corresponding discrete physical Laplacian is
\[
(\nabla_h^2 A)_{i,j}
=
\frac{1}{m_j^2}
\Delta_M^h A_{i,j}.
\]
Thus the discrete biharmonic friction acting on the stream function can be
computed as a physical Laplacian of the prognostic PV:
\[
(\nabla_h^4\psi^n)_{i,j}
=
(\nabla_h^2 q^n)_{i,j}
=
\frac{1}{m_j^2}
...
\frac{1}{m_j^2}
\left[
\frac{q_{i+1,j}^{n}-2q_{i,j}^{n}+q_{i-1,j}^{n}}{\Delta x^2}
+
\frac{q_{i,j+1}^{n}-2q_{i,j}^{n}+q_{i,j-1}^{n}}{\Delta y^2}
\right]
+
F_0\,S(t^n)
\exp
